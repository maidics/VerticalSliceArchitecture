...
- **No AutoMapper dependency**: Relies on explicit mapping to prevent hidden performance hits and simplify debugging.
...
Cross-cutting concerns are kept in a dedicated Pipeline folder, utilizing native endpoint filters:
```
Features/
└── Orders/                # The Feature boundary
├── Order.cs               # Domain Entity
├── OrderConfiguration.cs  # EF Core Entity Type Configuration
├── OrderEndpoints.cs      # Minimal API Endpoint definitions
